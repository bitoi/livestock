...
class FeedFacilityService:
    def __init__(self):
        self.db = MySQLConnector()

    def get_all(self, certified=False):
        query = """
        SELECT 
        ...
        WHERE f.type = 'FEED_PRODUCTION'
        """

        if certified:
            query += " AND f.certification IS NOT NULL AND f.certification <> ''"

        query += " ORDER BY f.id DESC"
        return self.db.fetch_all(query)

    def search(self, keyword, certified=False):
        query = """
        SELECT 
            f.id, f.name, f.address, f.phone, f.email,
            f.certification, f.scale, f.status,
    ...
    def create(self, data):
        ...
        VALUES (%s,%s,%s,%s,%s,'FEED_PRODUCTION',%s,%s,%s)
        """
