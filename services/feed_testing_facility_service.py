from database.mysql_connector import MySQLConnector

class FeedTestingFacilityService:
    def __init__(self):
        self.db = MySQLConnector()

    def get_all(self):
        query = """
        SELECT 
            f.id, f.name, f.address, f.phone, f.email,
            f.certification, f.scale, f.status,

            xa.id AS xa_id,
            xa.name AS xa_name,
            huyen.id AS huyen_id,
            huyen.name AS huyen_name,

            GROUP_CONCAT(food.name SEPARATOR ', ') AS foods
        FROM facility f
        LEFT JOIN administrative_unit xa ON f.unit_id = xa.id
        LEFT JOIN administrative_unit huyen ON xa.administrative_id = huyen.id
        LEFT JOIN facility_food ff ON f.id = ff.facility_id
        LEFT JOIN food ON ff.food_id = food.id
        WHERE f.type = 'FEED_TESTING'
        GROUP BY f.id
        ORDER BY f.id DESC
        """
        return self.db.fetch_all(query)

    def search(self, keyword):
        query = """
        SELECT 
            f.id, f.name, f.address, f.phone, f.email,
            f.certification, f.scale, f.status,

            xa.id AS xa_id,
            xa.name AS xa_name,
            huyen.id AS huyen_id,
            huyen.name AS huyen_name,

            GROUP_CONCAT(food.name SEPARATOR ', ') AS foods
        FROM facility f
        LEFT JOIN administrative_unit xa ON f.unit_id = xa.id
        LEFT JOIN administrative_unit huyen ON xa.administrative_id = huyen.id
        LEFT JOIN facility_food ff ON f.id = ff.facility_id
        LEFT JOIN food ON ff.food_id = food.id
        WHERE f.type = 'FEED_TESTING'
          AND f.name LIKE %s
        GROUP BY f.id
        """
        return self.db.fetch_all(query, (f"%{keyword}%",))

    def create(self, data, food_ids):
        query = """
        INSERT INTO facility
        (name, address, phone, email, certification,
         type, status, scale, unit_id)
        VALUES (%s,%s,%s,%s,%s,'FEED_TESTING',%s,%s,%s)
        """
        facility_id = self.db.execute_insert(query, (
            data["name"],
            data["address"],
            data["phone"],
            data["email"],
            data["certification"],
            data["status"],
            data["scale"],
            data["unit_id"]
        ))

        for food_id in food_ids:
            self.db.execute(
                "INSERT INTO facility_food (facility_id, food_id) VALUES (%s,%s)",
                (facility_id, food_id)
            )

    def update(self, facility_id, data, food_ids):
        query = """
        UPDATE facility
        SET name=%s, address=%s, phone=%s, email=%s,
            certification=%s, scale=%s, status=%s, unit_id=%s
        WHERE id=%s
        """
        self.db.execute(query, (
            data["name"],
            data["address"],
            data["phone"],
            data["email"],
            data["certification"],
            data["scale"],
            data["status"],
            data["unit_id"],
            facility_id
        ))

        self.db.execute(
            "DELETE FROM facility_food WHERE facility_id=%s",
            (facility_id,)
        )

        for food_id in food_ids:
            self.db.execute(
                "INSERT INTO facility_food (facility_id, food_id) VALUES (%s,%s)",
                (facility_id, food_id)
            )

    def delete(self, facility_id):
        self.db.execute("DELETE FROM facility_food WHERE facility_id=%s", (facility_id,))
        self.db.execute("DELETE FROM facility WHERE id=%s", (facility_id,))

    def get_by_id(self, facility_id):
        query = """
        SELECT 
            f.*,
            xa.id AS xa_id,
            huyen.id AS huyen_id
        FROM facility f
        LEFT JOIN administrative_unit xa ON f.unit_id = xa.id
        LEFT JOIN administrative_unit huyen ON xa.administrative_id = huyen.id
        WHERE f.id=%s
        """
        return self.db.fetch_one(query, (facility_id,))

    def get_food_ids(self, facility_id):
        rows = self.db.fetch_all(
            "SELECT food_id FROM facility_food WHERE facility_id=%s",
            (facility_id,)
        )
        return [r["food_id"] for r in rows]
