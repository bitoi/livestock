...
from views.feed_facility_view import FeedFacilityView
from dialogs.feed_facility_dialog import FeedFacilityDialog
from services.feed_facility_service import FeedFacilityService
from services.administrative_unit_service import AdministrativeUnitService
from services.activity_service import ActivityService
from auth.session import Session

class FeedFacilityController:
    def __init__(self):
        self.view = FeedFacilityView()
        self.service = FeedFacilityService()
    ...
    def load_data(self):
        certified = self.view.chk_certified.isChecked()
        data = self.service.get_all(certified)
        self._render_table(data)

    def search(self):
        keyword = self.view.txt_search.text()
        certified = self.view.chk_certified.isChecked()
        data = self.service.search(keyword, certified)
        self._render_table(data)

    def refresh(self):
        self.view.txt_search.clear()
        self.view.chk_certified.setChecked(False)
        self.load_data()

    # ================= ADD =================
    def add(self):
        dialog = FeedFacilityDialog()
        ...
        if dialog.exec():
            ...
            if Session.current_user:
                self.activity_service.log(
                    ...
                    f"Thêm cơ sở SX thức ăn chăn nuôi: {data['name']}"
                )
    ...
    def edit(self):
        ...
        dialog = FeedFacilityDialog(old_data)
        ...
        if dialog.exec():
            ...
            if Session.current_user:
                self.activity_service.log(
                    Session.current_user["id"],
                    f"Cập nhật cơ sở SX thức ăn chăn nuôi: {old_data['name']}"
                )
    ...
    def delete(self):
        ...
        if Session.current_user:
            self.activity_service.log(
                Session.current_user["id"],
                f"Xóa cơ sở SX thức ăn chăn nuôi: {name}"
            )
