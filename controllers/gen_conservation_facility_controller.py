...
from views.gen_conservation_facility_view import GenConservationFacilityView
from dialogs.gen_collection_facility_dialog import GenCollectionFacilityDialog
from services.gen_conservation_facility_service import GenConservationFacilityService
...
class GenConservationFacilityController:
    def __init__(self):
        self.view = GenConservationFacilityView()
        self.service = GenConservationFacilityService()
        ...
        if dialog.exec():
            ...
            if Session.current_user:
                self.log_service.log(
                    ...
                    f"Thêm cơ sở bảo tồn nguồn gen: {data['name']}"
                )
    ...
    def edit(self):
        ...
        if dialog.exec():
            ...
            if Session.current_user:
                self.log_service.log(
                    Session.current_user["id"],
                    f"Cập nhật cơ sở bảo tồn nguồn gen (ID={facility_id}): {update_data['name']}"
                )
    ...
    def delete(self):
        ...
        if Session.current_user:
            self.log_service.log(
                Session.current_user["id"],
                f"Xoá cơ sở bảo tồn nguồn gen (ID={facility_id}): {data['name']}"
            )
