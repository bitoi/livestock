...
from views.breeding_material_facility_view import BreedingMaterialFacilityView
from dialogs.breeding_material_facility_dialog import BreedingMaterialFacilityDialog

from services.breeding_material_facility_service import BreedingMaterialFacilityService
...
class BreedingMaterialFacilityController:
    def __init__(self):
        self.view = BreedingMaterialFacilityView()
        self.service = BreedingMaterialFacilityService()
    ...
    # ================= ADD =================
    def add(self):
        dlg = BreedingMaterialFacilityDialog()
    ...
    def save_add(self, dlg):
        ...
        if Session.current_user:
            self.log_service.log(
                ...
                f"Thêm cơ sở sản xuất tinh, phôi, ấu trùng, ấp trứng: {data['name']}"
            )
    ...
    def edit(self):
        ...
        dlg = BreedingMaterialFacilityDialog(data)
        self._load_huyen_xa(
            dlg,
    ...
    def save_edit(self, dlg, facility_id):
        ...
        if Session.current_user:
            self.log_service.log(
                Session.current_user["id"],
                f"Cập nhật cơ sở sản xuất tinh, phôi, ấu trùng, ấp trứng "
                f"(ID={facility_id}): {update_data['name']}"
            )
    ...
    def delete(self):
        ...
        if Session.current_user:
            self.log_service.log(
                Session.current_user["id"],
                f"Xoá cơ sở sản xuất tinh, phôi, ấu trùng, ấp trứng "
                f"(ID={facility_id}): {data['name']}"
            )
