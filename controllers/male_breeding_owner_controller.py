...
from views.male_breeding_owner_view import MaleBreedingOwnerView
from dialogs.male_breeding_owner_dialog import MaleBreedingOwnerDialog

from services.male_breeding_owner_service import MaleBreedingOwnerService
...
class MaleBreedingOwnerController:
    def __init__(self):
        self.view = MaleBreedingOwnerView()
        self.service = MaleBreedingOwnerService()
    ...
    # ================= ADD =================
    def add(self):
        dialog = MaleBreedingOwnerDialog()
        ...
        if dialog.exec():
            ...
            if Session.current_user:
                self.log_service.log(
                    ...
                    f"Thêm cơ sở sở hữu đực giống: {data['name']}"
                )
    ...
    def edit(self):
        ...
        owner_id = int(self.view.table.item(row, 0).text())
        data = self.service.get_by_id(owner_id)
        species_ids = self.service.get_species_ids(owner_id)

        dialog = MaleBreedingOwnerDialog(data, species_ids)
        self._load_huyen_xa(dialog)
        ...
        if dialog.exec():
            ...
            if Session.current_user:
                self.log_service.log(
                    Session.current_user["id"],
                    f"Cập nhật cơ sở sở hữu đực giống (ID={owner_id}): {update_data['name']}"
                )
    ...
    def delete(self):
        ...
        data = self.service.get_by_id(owner_id)
        self.service.delete(owner_id)

        if Session.current_user:
            self.log_service.log(
                Session.current_user["id"],
                f"Xoá cơ sở sở hữu đực giống (ID={owner_id}): {data['name']}"
            )
