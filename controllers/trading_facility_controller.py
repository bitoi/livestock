...
from views.trading_facility_view import TradingFacilityView
from dialogs.trading_facility_dialog import TradingFacilityDialog

from services.trading_facility_service import TradingFacilityService
...
class TradingFacilityController:
    def __init__(self):
        self.view = TradingFacilityView()
        self.service = TradingFacilityService()
    ...
    def save_add(self, dlg):
        ...
        if Session.current_user:
            self.log_service.log(
                ...
                f"Thêm cơ sở mua bán con giống: {data['name']}"
            )
    ...
    def edit(self):
        ...
        dlg = TradingFacilityDialog(data)
        self._load_huyen_xa(
            dlg,
    ...
    def save_edit(self, dlg, facility_id):
        ...
        if Session.current_user:
            self.log_service.log(
                Session.current_user["id"],
                f"Cập nhật cơ sở mua bán con giống (ID={facility_id}): {update_data['name']}"
            )
    ...
    def delete(self):
        ...
        if Session.current_user:
            self.log_service.log(
                Session.current_user["id"],
                f"Xoá cơ sở mua bán con giống (ID={facility_id}): {data['name']}"
            )
